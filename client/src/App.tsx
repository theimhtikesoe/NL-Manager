...
function MockUserRoute({ username, component: Component }: { username: string; component: React.LazyExoticComponent<React.ComponentType<{ user: any }>> }) {
  const { user } = useAuth();
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[100]">
      ...
      >
        ...
        {isOpen ? <ChevronDown className="h-3.5 w-3.5 text-zinc-500" /> : <ChevronUp className="h-3.5 w-3.5 text-zinc-500" />}
      </button>
    </div>
  );
}
