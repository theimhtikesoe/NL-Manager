...
export const machinesRelations = relations(machines, ({ one, many }) => ({
  assignedWorker: one(users, { fields: [machines.assignedWorkerId], references: [users.id] }),
  schedules: many(schedules),
  tasks: many(tasks),
}));

export const schedulesRelations = relations(schedules, ({ one }) => ({
  worker: one(users, { fields: [schedules.workerId], references: [users.id] }),
  machine: one(machines, { fields: [schedules.machineId], references: [machines.id] }),
...
export const tasksRelations = relations(tasks, ({ one, many }) => ({
  ...
  machine: one(machines, { fields: [tasks.machineId], references: [machines.id] }),
  proofs: many(taskProofs),
}));

export const taskProofsRelations = relations(taskProofs, ({ one }) => ({
  task: one(tasks, { fields: [taskProofs.taskId], references: [tasks.id] }),
  uploader: one(users, { fields: [taskProofs.uploadedBy], references: [users.id], relationName: "uploadedProofs" }),
  reviewer: one(users, { fields: [taskProofs.reviewedBy], references: [users.id] }),
}));
